...
<section class="content-main">
	<div class="content-header">
		 <a onclick="history.back()"><i class="material-icons md-arrow_back"></i> Go back </a>
			<div>
				 <h3 class="content-title card-title">Detail Cucian Online</h3>
			</div>
	</div>
	<div class="card mb-4">
		 ...
		 <div class="card-body">
			  <div class="row">
					...
					<div class="col-xl col-lg">
						...
						<p class="badge {{ $detail->status_bayar == 'belum dibayar' ? 'badge-danger' : 'badge-success' }}">{{ $detail->status_bayar }}</p>
					</div>
					<!--  col.// -->
					<div class="col-xl-6 text-md-end">
						@if ($detail->status_cucian == 'diproses')
						<a data-confirm="Cucian Online" onclick="actionConfirmation(this, event, 'Selesaikan Cucian {{ $detail->atas_nama }}')" href="{{ route('admin.cucian-online.selesaikan', $detail->no_order) }}" class="btn btn-success text-white">Selesaikan Cucian</a>
						@elseif ($detail->status_cucian == 'selesai')
						<a data-confirm="Cucian Online" onclick="actionConfirmation(this, event, 'Ambil Cucian {{ $detail->atas_nama }}')" href="{{ route('admin.cucian-online.diambil', $detail->no_order) }}" class="btn btn-primary">Diambil</a>
						@endif
						 <a href="{{ route('admin.cucian-online.edit', $detail->no_order) }}" class="btn btn-warning text-white">Edit</a>
						 <a data-confirm="Cucian Online" onclick="hapus(this, event)" href="{{ route('admin.cucian-online.hapus', $detail->no_order) }}" class="btn btn-danger">Hapus</a>
					</div>
					<!--  col.// -->
			  </div>
			  <!-- card-body.// -->
