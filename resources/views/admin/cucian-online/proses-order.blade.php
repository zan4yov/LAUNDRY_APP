...
<form action="{{ route('admin.cucian-online.proses-order.confirmation') }}" method="GET">
	<section class="content-main">
		<div class="row">
			 <div class="col-12">
				  <div class="content-header">
						<div>
							<h2 class="content-title">Proses Order Cucian Online</h2>
							<p>No. Order : {{ $cucian->no_order }}</p>
							<p>Nama User : {{ $cucian->user->nama }}</p>
							<p>Email User : {{ $cucian->user->email }}</p>
						</div>
						<div>
							 <button class="btn btn-md rounded font-sm hover-up">Proses Order</button>
						</div>
				  </div>
			 </div>
			 <div class="col-lg-6">
				  <div class="card mb-4">
					  <input type="hidden" id="no_order" name="no_order" value="{{ $cucian->no_order }}"/>
					  <input type="hidden" id="nama" name="nama" value="{{ $cucian->user->nama }}"/>
					  <input type="hidden" id="email" name="email" value="{{ $cucian->user->email }}"/>
						<div class="card-body">
							 <div class="mb-4">
